...
function export_pscad_lcp(
	...
	base_freq = f₀,
	file_name::String = "$(cable_system.case_id)_export.pscx",
	...
	for i in 1:num_cables
		...
		component_ids = collect(keys(cabledef.cable.components))
		num_cable_parts = length(component_ids)
		if num_cable_parts > 4
			error(
				"Cable $(cabledef.cable.cable_id) has $num_cable_parts parts, exceeding the limit of 4 (core/sheath/armor/outer).",
			)
		end
		conn = cabledef.conn
		...
		push!(coax1_params_vector, ("Name", "$(cabledef.cable.cable_id)"))
		...
		for (idx, component_id) in enumerate(component_ids)
			component = cabledef.cable.components[component_id]
	...
	# Assumes earth_props structure exists on cable_system
	earth_layer = cable_system.earth_props.layers[end]
...
$(_CLEANMETHODLIST(_serializable_fields))
"""
