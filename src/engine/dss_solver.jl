...
function get_Zint(ws, i::Int, k::Int, ::DeriModel)
    ...
    if abs(alpha) > 35.0
        ...
        @info "Using asymptotic approximation (abs(alpha) > 35.0) for I0/I1 ratio with alpha = $alpha"
    else
...
function get_Ze(ws, i::Int, j::Int, k::Int, ::DeriModel)
    ...
    vert_i = abs(ws.vert[i])
    if i == j
        h_term = vert_i + 1.0 / p_earth
        ln_arg = 2.0 * h_term
    else
        vert_j = abs(ws.vert[j])
        h_term = vert_i + vert_j + 2.0 / p_earth
...
function compute_impedance_matrix!(
    ...
    for i in 1:nph
        ...
        z_spacing =(formulation.earth_impedance == SimpleCarson() || formulation.earth_impedance == FullCarson() || formulation.earth_impedance == DeriModel()) ? L_factor * log(1.0 / ws.gmr[i]) : 0
        ...
        for j in 1:(i-1)
            ...
            z_spacing_mutual =  (formulation.earth_impedance == SimpleCarson() || formulation.earth_impedance == FullCarson() || formulation.earth_impedance == DeriModel()) ? L_factor * log(1.0 / d_ij) : 0
