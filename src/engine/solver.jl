...
# function compute!(problem::LineParametersProblem{T}, formulation::EMTFormulation) where {T}
# 	ws = init_workspace(problem, formulation)
# 	ComplexT = Complex{T}
# 	nph, nfreq = ws.n_phases, ws.n_frequencies

# 	# --- full matrices are built per slice (no 3D alloc) ----------------------
# 	Zbuf = Matrix{ComplexT}(undef, nph, nph)   # reordered scratch (mutated by merge_bundles!)
# 	Ybuf = Matrix{ComplexT}(undef, nph, nph)
# 	Ztmp = Matrix{ComplexT}(undef, nph, nph)   # raw slice coming from your builders
# 	Ytmp = Matrix{ComplexT}(undef, nph, nph)

# 	# --- index plan (constant across k) ---------------------------------------
# 	phase_map = ws.phase_map::Vector{Int}
# 	perm      = reorder_indices(phase_map)       # you already have this
...
# 	Zout = Array{ComplexT, 3}(undef, nkeep, nkeep, nfreq)
# 	Yout = Array{ComplexT, 3}(undef, nkeep, nkeep, nfreq)

# 	# tiny gather helper to avoid per-slice allocs
# 	@inline function _reorder_into!(dest::AbstractMatrix{ComplexT},
# 		src::AbstractMatrix{ComplexT},
