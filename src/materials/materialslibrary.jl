...
function _add_default_materials!(library::MaterialsLibrary)
	add!(library, "air", Material(Inf, 1.0, 1.0, 20.0, 0.0, 40.0, 1000.0, 0.0, 0.0))
	add!(library, "pec", Material(eps(), 1.0, 1.0, 20.0, 0.0, 0.0, 1000.0, 0.0, 0.0))
	add!(
		library,
		"copper",
		Material(1.7241e-8, 1.0, 0.999994, 20.0, 0.00393, 0.0025, 90.0, 0.0, 0.0),
	)
	add!(
		library,
		"aluminum",
		Material(2.8264e-8, 1.0, 1.000022, 20.0, 0.00429, 0.0042, 90.0, 0.0, 0.0),
	)
	add!(library, "xlpe", Material(1.97e14, 2.5, 1.0, 20.0, 0.0, 3.5, 90.0, 0.004, 0.4))
	add!(library, "pe", Material(1.97e14, 2.3, 1.0, 20.0, 0.0, 3.5, 70.0, 0.001, 0.4))
	add!(
		library,
		"semicon1",
		Material(1000.0, 1000.0, 1.0, 20.0, 0.0, 2.5, 90.0, 0.0, 0.0),
	)
	add!(
		library,
		"semicon2",
		Material(500.0, 1000.0, 1.0, 20.0, 0.0, 2.5, 90.0, 0.0, 0.0),
	)
	add!(
		library,
		"polyacrylate",
		Material(5.3e3, 32.3, 1.0, 20.0, 0.0, 6.0, 90.0, 0.0, 0.0),
	)
	add!(library, "lead", Material(21.4e-8, 1.0, 0.999983, 20.0, 0.00400, 35.7, 90.0, 0.0, 0.0)) # Lead or lead alloy
	add!(library, "steel", Material(13.8e-8, 1.0, 300.0, 20.0, 0.00450, 15.0, 90.0, 0.0, 0.0)) # Steel
	add!(library, "pp", Material(1e15, 2.8, 1.0, 20.0, 0.0, 6.0, 90.0, 0.001, 0.6)) # Laminated paper propylene
end
