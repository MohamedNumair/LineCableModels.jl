...
@testitem "simplify unit tests" setup = [defaults, deps_datamodel, defs_materials, simplify_fixtures] begin
    ...
    @testset "Input Validation" begin
        ...
        @test_throws MethodError DM.simplify()

        # Invalid first argument type
        @test_throws MethodError DM.simplify(42)

        # Invalid keyword type for new_id
        @test_throws TypeError DM.simplify(des; new_id=123)
    ...
    @testset "Basic Functionality" begin
        ...
        for (orig, simp) in zip(des.components, des_s.components)
            @test simp.conductor_group.radius_in ≈ orig.conductor_group.radius_in atol = TEST_TOL
            @test simp.conductor_group.radius_ext ≈ orig.conductor_group.radius_ext atol = TEST_TOL
            @test simp.insulator_group.radius_ext ≈ orig.insulator_group.radius_ext atol = TEST_TOL
            @test simp.id == orig.id
        end

        # new_id override
        des_s2 = DM.simplify(des; new_id="CAB-SIMPLE")
    ...
    @testset "Equivalence Preservation" begin
        ...
        for (orig, simp) in zip(des.components, des_s.components)
            ...
            @test simp.insulator_group.shunt_capacitance ≈ orig.insulator_group.shunt_capacitance atol = TEST_TOL
            @test simp.insulator_group.shunt_conductance ≈ orig.insulator_group.shunt_conductance atol = TEST_TOL
        end
    end
    ...
    @testset "Edge Cases" begin
        ...
        desM = DM.CableDesign(
            ...
            DM.CableComponent(
                des.components[1].id,
                DM.coerce_to_T(des.components[1].conductor_group, Measurements.Measurement{Float64}),
                DM.coerce_to_T(des.components[1].insulator_group, Measurements.Measurement{Float64}),
            ),
        )

        des_sM = DM.simplify(desM)
        @test typeof(des_sM.components[1].conductor_group.radius_in) <: Measurements.Measurement
        @test typeof(des_sM.components[1].conductor_group.radius_ext) <: Measurements.Measurement
        @test typeof(des_sM.components[1].insulator_group.radius_in) <: Measurements.Measurement
        @test typeof(des_sM.components[1].insulator_group.radius_ext) <: Measurements.Measurement
    end
    ...
    @testset "Type Stability & Promotion" begin
        ...
        @test typeof(sM.components[1].insulator_group.radius_ext) <: Measurements.Measurement
